// src/store-admin/src/datadog-rum.js
...
export function initDatadogRUM() {
  ...
  const service = process.env.VUE_APP_DATADOG_SERVICE || 'store-admin';
  ...
  datadogRum.init({
    ...
    // Proxy configuration (if needed for CORS)
    ...
    allowedTracingUrls: [
      ...
      { match: /\/makeline\/.*/, propagatorTypes: ['tracecontext'] },
      { match: /\/product.*/, propagatorTypes: ['tracecontext'] },
      { match: /\/order.*/, propagatorTypes: ['tracecontext'] },
      { match: /\/ai\/.*/, propagatorTypes: ['tracecontext'] }
    ],
